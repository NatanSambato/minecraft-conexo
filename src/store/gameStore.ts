...
const useGameStore = create<GameState>((set, get) => ({
  ...
  initGame: (puzzle, savedProgress) => {
    const allTiles = puzzle.groups.flatMap((g) =>
      g.items.map((label, i) => ({
        id: `${g.id}-${i}`,
        label,
        groupId: g.id,
  ...
  activateHint: () => {
    ...
    const unsolvedGroups = puzzle!.groups.filter(
      (g) => !solvedGroups.some((sg) => sg.id === g.id),
    );
    const newHintedGroups = { ...hintedGroups };
    const partialGroup = unsolvedGroups.find(
      (g) => hintedGroups[g.id] && hintedGroups[g.id].length < 3,
    );

    if (partialGroup) {
      const alreadyHinted = hintedGroups[partialGroup.id];
      const candidates = tiles.filter(
        (t) => t.groupId === partialGroup.id && !alreadyHinted.includes(t.id),
      );
  ...
  saveProgress: () => {
    ...
    if (!puzzle) return;
    saveProgressToStorage(puzzle.id, {
      status,
      solvedGroups,
      attempts,
      hintsUsed,
